...
def scene_downloader(scene_list=None, id=None, request=None, sat_choice=None):
    for scene in scene_list:
        if sat_choice == '1':
            ...
            if id==0:
                os.system(
                    f"landsatxplore download {scene} --output /home/chiko/Storage/Projects/Raster_Image_Calculator/Images/Request.{request}/Start_date --username {username} --password {password}")
            elif id == 1:
                os.system(
                    f"landsatxplore download {scene} --output /home/chiko/Storage/Projects/Raster_Image_Calculator/Images/Request.{request}/End_date --username {username} --password {password}")
...
def download_selector(scenes=None, test=False, sat_choice=None):
    ...
    if sat_choice == '1':
        ...
        columns = ['entity_id', 'tile_number', 'cloud_cover']
    for scene in scenes:
